...
import type { CollectionWithBooks } from "@/lib/types";
...
/* ── Helper: extract colors from a collection's books ── */

const FALLBACK_COLOR = "#333";

function getStackColors(collection: CollectionWithBooks): {
  color1: string;
  color2: string;
  color3: string;
} {
  ...
  return {
    color1: books[0]?.bgColor ?? FALLBACK_COLOR,
    color2: books[1]?.bgColor ?? FALLBACK_COLOR,
    color3: books[2]?.bgColor ?? FALLBACK_COLOR,
  };
}

/* ── Right-side positioning for book stacks ── */

const STACK_POSITIONS = [
  "absolute right-[240px] bottom-[100%] z-30",
  "absolute right-[130px] bottom-[100%] z-30",
  "absolute right-10 bottom-[100%] z-30",
];
        ...
        <div className="relative w-full max-w-[1200px] px-6 z-10 pt-10 flex flex-col items-center">
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="relative">
              ...
              {/* Right — dynamic book stacks from collections */}
              {collections.slice(0, 3).map((collection, index) => {
                const { color1, color2, color3 } = getStackColors(collection);
                return (
                  <div key={collection.id} className={STACK_POSITIONS[index]}>
                    <ClickableBookStack
